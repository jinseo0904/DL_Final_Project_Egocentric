...
def train_quick_test(args):
    ...
    if hasattr(args, 'use_class_weights') and args.use_class_weights:
        ...
        class_counts = torch.tensor([
            35, 34, 15, 61, 3, 19, 74, 50, 29, 26, 25, 16, 6, 92, 19
        ], dtype=torch.float32)  # Update with your actual counts
    ...
    # Replace current scheduler with this
    ...
    for epoch in range(args.epochs):
        ...
        if epoch_val_acc > best_val_acc:
            ...
            best_path = checkpoint_dir / f"model_best.pth"
            torch.save(checkpoint, best_path)
            print(f"✅ New best model saved with val_acc: {epoch_val_acc:.4f}")
        
        # Save periodic checkpoints
        if (epoch + 1) % args.save_freq == 0 or epoch == args.epochs - 1:
            epoch_path = checkpoint_dir / f"model_epoch_{epoch+1}.pth"
    ...
    print(f"Model saved to quick_test_model_15.pth")
